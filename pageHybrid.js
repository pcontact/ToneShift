...
(function () {
  ...
  window.addEventListener("message", async (event) => {
    ...
    const { textWithPlaceholders, textWithoutPlaceholders, rewriteWithFormat, context, tone, complexity, brevity } = event.data;
    const profile = {tone, complexity, brevity}
    _rewriteWithFormat = rewriteWithFormat

    const freeWritePrompt = buildPromptContextAwareRewrite(textWithoutPlaceholders, context, profile)
...
function buildPromptContextAwareRewrite(textWithoutPlaceholders, pageContext, { tone, complexity, brevity }) {
  return `
...
async function summarizeText(text) {
 ...
 // fallback Gemini if needed
  ...
  // TODO: replace with actual AI summarizer
  return "This is a dummy placeholder summary for " + text;
}
