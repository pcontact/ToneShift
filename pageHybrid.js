(function () {
    ...
    function getApiKey() {
        return new Promise((resolve) => {
            // Ask content script for API key
  ...
  // Core rewrite logic (shared prompt)
  ...
  async function tryChromeAI(text, tone, complexity, brevity) {
    ...
    // Use Gemini Cloud API as fallback
    async function tryGeminiCloud(promptText) {
        try {
            ...
            message = "⚠️ No API key found. Please open the ToneShift popup and add your Gemini API key.";
            }
            console.error("Gemini error:", err);
            window.postMessage({ type: "TS_GEMINI_RESPONSE", text: message }, "*");
        }
    }
  ...
  window.addEventListener("message", async (event) => {
    ...
    if (!output) {
      output = await tryGeminiCloud(text, tone, complexity, brevity);
    }
